...
module.exports.run = async (bot, message, args, connection) => {
	...
	if(!add) return message.channel.send("Veuillez donner une somme à donner ou à retirer").then(m => m.delete(5000));
	const timeout = 1500;
	...
	connection.query(`SELECT * FROM user WHERE id = '${target.id}'`, (err, rows) => {
		...
		if(rows.length < 1) {
			...
			sql = `UPDATE user SET coins = ${acoins + add} WHERE id = '${target.id}'`;
		}
	...
	setTimeout(function(){
		connection.query(`SELECT * FROM user WHERE id = '${target.id}'`, (err, rows) => {
			...
			msg.delete();
			message.channel.send(new RichEmbed().setDescription(target + " Vous avez obtenu **" + add + "** Coin(s). \n Vous avez maintenant **" + bcoins + "** Coin(s) ! :moneybag:").setColor("#daa520"));
		});
	 }, timeout);
